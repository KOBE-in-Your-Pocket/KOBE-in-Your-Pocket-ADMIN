...
| 画面 | データ |
|---|---|
| スポット / レビュー / ユーザー / ジャンル / ダッシュボード | 実 API |
| マナー / 避難所 / 統計 / 操作ログ | 準備中画面 |
...
デモで配ったアカウントは **本番 Backend に対する本物の権限**を持つ。放置すると、
資料やチャットに残った認証情報から誰でもログインできる状態が続く。悪意の有無に関わらず、
興味本位の操作でスポットや避難所が消える（避難所は Flyway seed の再実行では戻らない）。

一時的に配ったものは用が済んだら失効させる。デモが終わったら必ず実施すること。
